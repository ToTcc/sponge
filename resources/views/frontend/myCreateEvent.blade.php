	...
	<style type="text/css">
		...
		*{
		...
		@media (max-width: 414px){
			...
			.qr-code{
				background: url('/assets/frontend/image/qr-code.png')no-repeat center;
			...
			var app = new Vue({
				...
				methods: {
					...
					viewDetail: function(id) {
						window.location.href = "/viewMyCreateEventDetail/" + id;
					},
					viewComment: function(id) {
						window.location.href = "/viewMyCreateActivityComment/" + id;
					}
				}
			})
	...
	<div class="my-same">
		<div class="header">
			...
			<div class="nav-flex-2">
				...
				<div class="item-2" v-on:click="myJoinEvent"><div class="seated"></div><div>我参加的</div></div>
				<div class="item-2 active"><div class="seated active"></div><div>我发起的</div></div>
			</div>
			<div class="my-same-info">
				<div class="has-same">共发起<span class="color-blue color-bold font"> {{$data["total_counts"]}} </span>个</div>
				@if($data["info"]["available"] == config("enumerations.CUSTOMER_AVAILABLE.BLACK"))
				<div class="go-on color-red">暂停发起活动</div>
				@endif
			</div>
		</div>
		<div class="create-content">
			@if($data["info"]["available"] == config("enumerations.CUSTOMER_AVAILABLE.NORMAL"))
				@if($data["apply_count"] < 1)
					<div class="no-seat">
						<div class="no-seat-word">你还没有发起过活动，快来发起吧！</div>
						<div class="button" v-on:click="gotoApply">
							<p>成为发起人</p>
						</div>
					</div>
				@elseif($data["apply_list"][0]["status"] == 0)
					<div class="no-seat">
						<div class="no-seat-word color-blue">发起人身份审核中，请耐心等耐！</div>
					</div>
				@else
					<div class="no-seat">
						<div class="no-seat-refuse">发起人身份审核被拒绝，拒绝理由如下：</div>
						<div class="reason">{{$data["apply_list"][0]["reject_reason"]}}</div>
						<div class="line" style="width:92%;margin: 0 auto;"></div>
						<div class="no-seat-reset">你也可以重新成为发起人。</div>
						<div class="button" v-on:click="gotoApply">
							<p>成为发起人</p>
						</div>
						<div class="no-seat-reset">或联系客服</div>
				...
				@else
					@foreach($data["list"] as $item)
						...
						@if($item["status"] == config("enumerations.EVENT_STATUS.WAITING_JOIN")
								|| $item["status"] == config("enumerations.EVENT_STATUS.FULL"))
						...
						@elseif($item["status"] == config("enumerations.EVENT_STATUS.END"))
							<div class="create-order-details" v-on:click="viewComment({{$item["event_id"]}})">
						@elseif($item["status"] == config("enumerations.EVENT_STATUS.CANCEL"))
						...
						<div class="head-img-pic"  style="background:url('{{$item["title_image"]}}') no-repeat center;background-size: cover;"></div>
							<div class="has-pay-info">
								...
								@if($item["status"] == config("enumerations.EVENT_STATUS.WAITING_JOIN")
									|| $item["status"] == config("enumerations.EVENT_STATUS.FULL"))
									<div class="moive-status-begin"><span class="color-blue join-num">{{$item["join_number"]}}</span>/{{$item["upper_limit"]}}</div>
								@elseif($item["status"] == config("enumerations.EVENT_STATUS.END"))
									<div class="moive-status-sign color-blue">查看反馈</div>
								@elseif($item["status"] == config("enumerations.EVENT_STATUS.CANCEL"))
									<div class="moive-status-sign color-blue">平台取消</div>
								@endif
							</div>
							<div class="line"></div>
						</div>
