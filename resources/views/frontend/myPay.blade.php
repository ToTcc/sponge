				...
				var app = new Vue({
					...
					methods: {
						...
						gotoSign: function(id) {
							...
							window.location.href = "/activitySignPage/" + id;
						},
						gotoComment: function(id) {
							event.stopPropagation();
							window.location.href = "/commentActivityPage/" + id;
						},
						qrcode: function(id) {
							window.location.href = "/activityQrcode/" + id;
						}
					}
				})
		...
		<div class="my-same">
			<div class="header">
				...
				<div class="nav-flex-2">
					...
					<div class="item-2" v-on:click="myLike"><div class="seated"></div><div>已占座的</div></div>
					<div class="item-2 active"><div class="seated active"></div><div>已付款的</div></div>
				</div>
				<div class="my-same-info">
					<div class="has-same">共付款<span class="color-blue color-bold font"> {{$data["total_counts"]}} </span>次</div>
					<div class="go-on color-red" v-on:click="search">查看近期放映</div>
				</div>
			</div>
			<div class="create-content">
				...
				@if($data["total_counts"] == 0)
					...
					<div class="no-seat hide">
						<div class="no-seat-word">你还没有来看过电影呦，看看近期的放映吧！</div>
				...
				@else
					...
					@foreach($data["list"] as $item)
						...
						@if($item["status"] == config("enumerations.ACTIVITY_JOIN_STATUS.END_PAY"))
							<div class="create-order-details">
								<div class="head-img-pic" style="background:url('{{$item["info"]["images"]["large"]}}') no-repeat center;background-size: cover;"></div>
								<div class="has-pay-info">
									<p class="pay-info-title">{{$item["info"]["title"]}}</p>
									<p class="pay-info-time">{{$item["time"]}}</p>
									{{$item["is_open"]}}
									@if(!$item["is_open"])
										<div class="moive-status-begin color-blue" v-on:click="qrcode({{$item['activity_id']}})" style="top:0;">等待开始</div>
										<div class="moive-status-begin color-blue" v-on:click="qrcode({{$item['activity_id']}})" style="top:15px;">加群活动</div>
										<p class="code" style="top:35px;" v-on:click="movieInfo({{$item['douban_id']}})"></p>
									@else
										<div class="moive-status-begin color-blue" v-on:click="gotoSign({{$item["activity_id"]}})">签到</div>
										<p class="code" v-on:click="gotoSign({{$item["activity_id"]}})" style="top:35px;"></p>
									@endif
								</div>
								<div class="line"></div>
							</div>
						@elseif($item["status"] == config("enumerations.ACTIVITY_JOIN_STATUS.END_SIGN"))
							<div class="create-order-details" v-on:click="movieInfo({{$item['douban_id']}})">
								<div class="head-img-pic" style="background:url('{{$item["info"]["images"]["large"]}}') no-repeat center;background-size: cover;"></div>
								<div class="has-pay-info">
									<p class="pay-info-title">{{$item["info"]["title"]}}</p>
									<p class="pay-info-time">{{$item["time"]}}</p>
									<div class="moive-status-comment color-blue" v-on:click="gotoComment({{$item["activity_id"]}})">请反馈</div>
								</div>
								<div class="line"></div>
							</div>
						@elseif($item["status"] == config("enumerations.ACTIVITY_JOIN_STATUS.END_COMMENT"))
							<div class="create-order-details" v-on:click="movieInfo({{$item['douban_id']}})">
								<div class="head-img-pic" style="background:url('{{$item["info"]["images"]["large"]}}') no-repeat center;background-size: cover;"></div>
								<div class="has-pay-info">
									<p class="pay-info-title">{{$item["info"]["title"]}}</p>
						...
						@elseif($item["status"] == config("enumerations.ACTIVITY_JOIN_STATUS.REFUND"))
							<div class="create-order-details" v-on:click="movieInfo({{$item['douban_id']}})">
								<div class="head-img-pic" style="background:url('{{$item["info"]["images"]["large"]}}') no-repeat center;background-size: cover;"></div>
								<div class="has-pay-info">
									<p class="pay-info-title">{{$item["info"]["title"]}}</p>
									<p class="pay-info-time">{{$item["time"]}}</p>
									<div class="moive-status-hascomemnt" style="color:#ff9d9d;">已请假</div>
								</div>
								<div class="line"></div>
							</div>
						@endif
